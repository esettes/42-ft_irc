...
static void testTopicQueryIgnoresTopicRestriction() {
    ...
    if (operatorSocketFd == -1 || memberSocketFd == -1) {
        reportFailure(
            ...
            "for phase 14 TOPIC query privilege tests",
            "successful connections",
            "connection failed");
    ...
    expectEqual(
        ...
        ":irc.42.local 332 roxana #general "
        ":Solo operadores pueden cambiarlo\r\n",
        "Phase 14: a regular member should "
        "still be able to query the topic with +t");
