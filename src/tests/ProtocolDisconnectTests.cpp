...
static void testDisconnectUnregisteredClient() {
    ...
    expectEqual(
        ...
        "Phase 19: the server should accept"
        "a new client after an unregistered disconnect");

    expectTrue(
        server.isRunning(),
        "Phase 19: disconnecting an unregistered"
        "client should not stop the server",
...
static void testDisconnectFromMultipleChannelsSendsOneQuit() {
    ...
    if (!extractClientPrefixFromWelcome(
        ...
        reportFailure(
            ...
            "prefix for phase 19 multi-channel QUIT tests",
    ...
    expectEqual(
        bobQuit,
        expectedQuit,
        "Phase 19: a peer sharing two"
        "channels should receive exactly one QUIT");
    expectEqual(
        carolQuit,
        expectedQuit,
...
static void testUnexpectedTcpCloseNotifiesPeers() {
    ...
    if (!extractClientPrefixFromWelcome(
        ...
        reportFailure(
            "Welcome should expose a"
            "prefix for phase 19 unexpected close tests",
            "welcome containing alice!alice@host",
            aliceWelcome);
        closeSocket(aliceSocketFd);
        closeSocket(bobSocketFd);
    ...
    expectEqual(
        ...
        "PONG :after-hup\r\n",
        "Phase 19: a remaining member"
        "should stay connected after a peer hangup");
