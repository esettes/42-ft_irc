...
static void testFileDescriptorExhaustionDoesNotStopServer() {
    ...
    expectEqual(
        ...
        "should still be served after fd exhaustion");
    ...
    expectContains(
        ...
        "001",
        "A new client should register after"
        "the file descriptor limit is no longer exceeded");

    expectTrue(
        server.isRunning(),
        "Releasing connections after"
        "fd exhaustion should keep the server running",
        "ircserv remains running",
        "ircserv exited");
