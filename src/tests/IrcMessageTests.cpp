...
static void expectEqual(const std::string &actual,
...
static void testSerializeWithPrefixAndMultipleParameters() {
    ...
    expectEqual(message.serialize(),
        ...
        "serialize should include "
        "prefix and use trailing parameter for the last value");
}
...
static void testRejectsInvalidCommand() {
    ...
    try {
        message.serialize();
        expectTrue(false,
            "serialize should reject commands containing whitespace");
    }
...
static void testRejectsCommandWithControlCharacters() {
    ...
    try {
        message.serialize();
        expectTrue(false,
            "serialize should reject commands containing carriage return");
    }
    catch (const std::runtime_error &) {
        expectTrue(true,
            "serialize rejected invalid command with carriage return");
    }
}
...
static void testRejectsInvalidPrefix() {
    ...
    try {
        message.serialize();
        expectTrue(false,
            "serialize should reject prefixes containing newlines");
    }
    catch (const std::runtime_error &) {
...
static void testRejectsInvalidParameter() {
    ...
    try {
        message.serialize();
        expectTrue(false,
            "serialize should reject parameters containing NUL bytes");
    }
    catch (const std::runtime_error &) {
...
static void testRejectsMessageExceedingMaxLength() {
    ...
    expectSerializationFailure(
        message,
        "serialize should reject messages"
        " longer than 512 bytes including CRLF");
}
