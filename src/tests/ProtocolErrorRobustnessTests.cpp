...
static void testRegistrationProtocolErrors()
{
    ...
    expectEqual(
        ...
        ":irc.42.local 433 challenger existingNick :Nickname is already in use\r\n",
...
static void testChannelMessageAndOperatorProtocolErrors()
{
    ...
    expectEqual(
        ...
        "Phase 19: rejected operator commands should not apply partial MODE state"
    );

    expectEqual(
        sendLineAndReceive(operatorSocketFd, "PING :after-errors\r\n"),
        "PONG :after-errors\r\n",
        "Phase 19: the client should keep sending commands after protocol errors"
    );

    expectTrue(
        server.isRunning(),
        "Phase 19: invalid channel and operator commands should not stop the server",
        "ircserv remains running",
        "ircserv exited"
    );
